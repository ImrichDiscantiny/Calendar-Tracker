...
export function Navbar() {
  ...
  const UserIcon = FaUserAlt as FunctionComponent<IconBaseProps>;

  return (
    <nav className="bg-[#0f285f] flex ">
      ...
      <NavLink className="ml-auto" to="/user">
        {({isActive}) => (
          <div className=" p-1 px-2 flex flex-row justify-between gap-1 items-center">
            {createElement(UserIcon, {size: 16, color: '#fff', className: 'my-auto'})}
            <span className={isActive ? 'active-span font-semibold' : 'text-[#fff]'}>UserName</span>
          </div>
        )}
      </NavLink>
    </nav>
